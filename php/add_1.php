...
include "../db_ip.php";
//system("sudo sh sh/add_tags.sh");
//system("sudo insmod /var/www/test/php/sh/check_tags.ko");
...
$collection->update(array("cmd"=>"add1"),array('$set'=>array("srvId"=>$_POST["srvid"],"dpi"=>$_POST["dpi"],"des"=>$_POST["des"])));
$collection = $dbc->List->node;

//$user = "admin";
//$pass = "123456";

$file = fopen("conf/add_1.conf","r");
$arr = array();
while(!feof($file))
{
	$line = fgets($file);
...
$dpi->id = $info["id"];
	...
	$temp = new CmdTxRxICE_RespAddSmpAcl;
	...
	$temp = $ic->mccnAddSmpAclI($cmd);

	//log
$collection = $dbc->List->cmd;
$collection->update(array("cmd"=>"add1"),array('$set'=>array("result"=>array("exCode"=>$temp->exInfo->exCode,"rstCode"=>$temp->rstCode))));
